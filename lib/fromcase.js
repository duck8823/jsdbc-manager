...
class FromCase {
	...
	list(callback) {
		...
		this._adaptor.fetchAll(util.format('SELECT %s FROM %s %s', columns, table, where), (err, rows) => {
	...
	delete() {
		return new Executable(this._adaptor, util.format('DELETE FROM %s %s', this._entity.name, this._where ? this._where : ''));
	}
}
