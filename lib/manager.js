...
class PjdbcManager {
	...
	from(entity) {
		return new FromCase(this, entity);
	}

	drop(entity) {
		return new Executable(this, util.format('DROP TABLE IF EXISTS %s', entity.name));
	}
	...
	create(entity) {
		...
		return new Executable(this, util.format('CREATE TABLE %s (%s)', entity.constructor.name, columns.join(', ')))
	}

	insert(data) {
		return new Executable(this, util.format('INSERT INTO %s %s', data.constructor.name, _createSentence(data)))
	}
}
